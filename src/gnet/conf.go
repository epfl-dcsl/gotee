...
func initConfVal() {
	...
	if confVal.dnsDebugLevel > 0 {
		defer func() {
			...
			case confVal.netGo:
				if netGo {
					println("go package gnet: built with netgo build tag; using Go's DNS resolver")
				} else {
					println("go package gnet: GODEBUG setting forcing use of Go's resolver")
				}
			case confVal.forceCgoLookupHost:
				println("go package gnet: using cgo DNS resolver")
			default:
				println("go package gnet: dynamic selection of DNS resolver")
			}
		}()
	}
...
func (c *conf) hostLookupOrder(hostname string) (ret hostLookupOrder) {
	if c.dnsDebugLevel > 1 {
		defer func() {
			print("go package gnet: hostLookupOrder(", hostname, ") = ", ret.String(), "\n")
		}()
	}
...
// goDebugNetDNS parses the value of the GODEBUG "gnetdns" value.
