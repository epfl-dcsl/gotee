...
func (eai addrinfoErrno) Error() string   { return "error" /*C.GoString(C.gai_strerror(C.int(eai)))*/ }
func (eai addrinfoErrno) Temporary() bool { return eai == EAI_AGAIN }
...
func cgoLookupServicePort(hints *struct_addrinfo, network, service string) (port int, err error) {
	...
	var res *struct_addrinfo
	gerrno, err := getaddrinfo(nil, unsafe.Pointer(&cservice[0]), hints, &res)
	if gerrno != 0 {
		switch gerrno {
		case EAI_SYSTEM:
...
func cgoPortLookup(result chan<- portLookupResult, hints *struct_addrinfo, network, service string) {
	port, err := cgoLookupServicePort(hints, network, service)
...
func cgoLookupIPCNAME(name string) (addrs []IPAddr, cname string, err error) {
	...
	copy(h, name)
	var res *struct_addrinfo
	gerrno, err := getaddrinfo(unsafe.Pointer(&h[0]), nil, &hints, &res)
	if gerrno != 0 {
		switch gerrno {
		case EAI_SYSTEM:
			if err == nil {
	...
	if res != nil {
		cname = "" //C.GoString(res.ai_canonname) TODO aghosn
		if cname == "" {
			cname = name
		}
...
func cgoLookupAddrPTR(addr string, sa *struct_sockaddr, salen socklen_t) (names []string, err error) {
	acquireThread()
	defer releaseThread()
	...
	for l := nameinfoLen; l <= maxNameinfoLen; l *= 2 {
		...
		if gerrno == 0 || gerrno != EAI_OVERFLOW {
			break
		}
	}
	if gerrno != 0 {
...
func cgoReverseLookup(result chan<- reverseLookupResult, addr string, sa *struct_sockaddr, salen socklen_t) {
	names, err := cgoLookupAddrPTR(addr, sa, salen)
	result <- reverseLookupResult{names, err}
}

func cgoSockaddr(ip IP, zone string) (*struct_sockaddr, socklen_t) {
	if ip4 := ip.To4(); ip4 != nil {
		return cgoSockaddrInet4(ip4), syscall.SizeofSockaddrInet4
	}
	if ip6 := ip.To16(); ip6 != nil {
		return cgoSockaddrInet6(ip6, zoneCache.index(zone)), syscall.SizeofSockaddrInet6
	}
	return nil, 0
}
